...
def train_lstm(dataset, output_dir, epochs, warmup_steps, learning_rate, weight_decay, gradient_accumulation_steps,
    ...
    for epoch in range(epochs):
        epoch_iterator = tqdm(train_loader, desc=f"Epoch {epoch + 1}", leave=False)
        for step, batch in enumerate(epoch_iterator):
            ...
            # Print each sample and its label in the current batch
            for i in range(len(flat_docs)):
                sample_text = tokenizer.decode(flat_docs[i])
                label_id = labels[i].item()
                logging.info(f"Train Sample: '{sample_text}' | Label: '{label_dict[label_id]}'")
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % gradient_accumulation_steps == 0:
                ...
                # Intermediate logging during training
        ...
        logging.info(f"Epoch {epoch} Training Loss: {epoch_training_loss}")

        # Evaluate the model on the test data
        acc, eval_loss = evaluate_model(model, test_data, eval_batch_size, device, output_dir)
        logging.info(f"Epoch {epoch} Evaluation Loss: {eval_loss} Accuracy: {acc}")

    # Final evaluation after all epochs are complete
    acc, eval_loss = evaluate_model(model, test_data, eval_batch_size, device, output_dir)
    logging.info(f"Final Evaluation Loss: {eval_loss}")
    logging.info(f"Final Evaluation Accuracy: {acc}")
...
def main():
    ...
    if args.model == "MLP":
        print("MLP")
        label_dict = {i: label for i, label in enumerate(dataset['train'].get_labels())}
        tokenizer = AutoTokenizer.from_pretrained(MODELS["MLP"])
        train_mlp(dataset,
                  args.output_dir,
